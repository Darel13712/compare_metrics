...
RecSys2019_DL_Eval | - | есть (в формуле DCG в знаменателе логарифм [натуральный](https://github.com/MaurizioFD/RecSys2019_DeepLearning_Evaluation/blob/c173c9688c12e8b2866ac5f707e555faf811996f/Base/Evaluation/metrics.py#L180), а не по основанию 2. Для рассчета iDCG берутся все GT relevances, не обрезанные по K) 
DaisyRec | есть (используется формула с 2 в числителе, но перед подачей в формулу релевантности [бинаризуются](https://github.com/AmazingDD/daisyRec/blob/1b7d693f07b892f26eb026d416d86a59a6c3e3a0/daisy/utils/metrics.py#L126)) | -
RecSysPytorch | есть | -
NeuRec | есть | -
rs_metrics  | есть | -
suprise  | - | -
...
1) GAUC (Group AUC): считаем AUC для каждого пользователя отдельно, взвешенное среднее AUC'ов пользователей. https://arxiv.org/pdf/1703.02091.pdf
...
4) LAUC@K (Limited AUC): , усредняет по пользователям. Такой же, как в 4м пункте, но positives ограничиваются топом, в 3м positives берутся все.
5) GAUC@K: в GAUC подается только топ K элементов, и ответы для них.
...
Библиотека | GAUC | SAUC | LAUC@K | GAUC@K
------------ | ------------- | ------------ | ------------- | ------------
Replay | есть | - | - | - 
ReсBole | есть (скипает AUC'и для пользователей без TP) | есть | - | - 
...
- SAUC у Elliot отличается по реализации от других, отсюда отличие в скорах.
- GAUC у Elliot получается усреднением, а в RecBole усреднением со взвешиванием.
- GAUC@20 в RecSys2019_DL_Eval считает обратный AUC для пользователей.
